package com.andruhavuho.man.like.notes.ui

import android.os.Bundle
import android.view.Menu
import android.view.MenuInflater
import android.view.MenuItem
import android.view.View
import android.widget.TextView
import android.widget.Toast
import androidx.core.os.bundleOf
import androidx.fragment.app.Fragment
import androidx.lifecycle.viewModelScope
import com.andruhavuho.man.like.notes.App
import com.andruhavuho.man.like.notes.MainActivity
import com.andruhavuho.man.like.notes.R
import com.andruhavuho.man.like.notes.database.AppDatabase
import com.andruhavuho.man.like.notes.database.NotesViewModel
import kotlinx.coroutines.launch

class NoteInfoFragment : Fragment(R.layout.fragment_note_info) {

    private var currentNoteId: Long = 0

    private lateinit var tvNoteTitle: TextView
    private lateinit var tvNoteDescription: TextView

    private lateinit var vm : NotesViewModel

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        setHasOptionsMenu(true)

        vm = (requireActivity().application as App).vm

        arguments?.getLong(NOTE_ID)?.takeIf { it > 0 }?.let { noteId ->
            currentNoteId = noteId
        } ?: run {
            Toast.makeText(requireActivity(), "Something Went Wrong", Toast.LENGTH_LONG).show()
            requireActivity().onBackPressed()
        }

        tvNoteTitle = view.findViewById(R.id.tvNoteTitle)
        tvNoteDescription = view.findViewById(R.id.tvNoteDescription)

        vm.viewModelScope.launch {
            val note = vm.getNote(currentNoteId).toUserNote()
            tvNoteTitle.text = note.getNameOrDefault()
            tvNoteDescription.text = note.getDescriptionOrDefault()
        }
    }

    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        super.onCreateOptionsMenu(menu, inflater)
        inflater.inflate(R.menu.menu_note_info, menu)
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.mi_edit -> {
                (requireActivity() as MainActivity).showEditNoteFragment(currentNoteId)
                return true
            }
        }
        return super.onOptionsItemSelected(item)
    }

    companion object {

        const val NOTE_ID = "NOTE_ID"

        fun newInstance(noteId: Long): NoteInfoFragment {
            return NoteInfoFragment().apply {
                arguments = bundleOf(NOTE_ID to noteId)
            }
        }
    }
}